...
namespace TraitSharp.SourceGenerator.Generators
{
    internal static class TraitSpanFactoryGenerator
    {
        ...
        public static string Generate(TraitModel trait)
        {
            ...
            builder.AppendLine("/// </summary>");
            builder.AppendLine($"public static partial class {trait.Name}SpanExtensions");
            builder.OpenBrace();
        ...
        /// <summary>
        /// Generates concrete (non-generic) span factory overloads for a specific implementation
        /// that has BaseOffset == 0. These overloads return the two-parameter span types
        /// (e.g., TraitSpan&lt;TLayout, TSource&gt;) where the JIT can constant-fold sizeof(TSource),
        /// achieving native Span&lt;T&gt; performance parity.
        /// C# overload resolution prefers these concrete overloads over the generic ones.
        /// </summary>
        public static string? GeneratePerImplementation(ImplementationModel impl)
        {
            var trait = impl.Trait!;
            if (impl.BaseOffset != 0) return null;

            var builder = new CodeBuilder();
            var shortName = trait.ShortName;
            var layoutType = $"{trait.EffectiveNamespace}.{trait.LayoutStructName}";
            var sourceType = impl.FullTypeName;
            ...
            // --- 1D ReadOnly: AsXxxSpan from ReadOnlySpan<TSource> ---
            builder.AppendLine("/// <summary>");
            builder.AppendLine($"/// Creates a read-only trait span viewing {trait.Name} fields (optimized for {impl.TypeName}).");
            builder.AppendLine("/// </summary>");
            builder.AppendLine("[MethodImpl(MethodImplOptions.AggressiveInlining)]");
            builder.AppendLine($"public static ReadOnlyTraitSpan<{layoutType}, {sourceType}> As{shortName}Span(");
            builder.AppendLine($"    this ReadOnlySpan<{sourceType}> source)");
            builder.OpenBrace();
            builder.AppendLine($"return new ReadOnlyTraitSpan<{layoutType}, {sourceType}>(");
            builder.AppendLine($"    ref MemoryMarshal.GetReference(source),");
            builder.AppendLine($"    source.Length);");
            builder.CloseBrace();
            builder.AppendLine();

            // --- 1D ReadOnly: AsXxxSpan from Span<TSource> ---
            builder.AppendLine("/// <summary>");
            builder.AppendLine($"/// Creates a read-only trait span viewing {trait.Name} fields (optimized for {impl.TypeName}).");
            builder.AppendLine("/// </summary>");
            builder.AppendLine("[MethodImpl(MethodImplOptions.AggressiveInlining)]");
            builder.AppendLine($"public static ReadOnlyTraitSpan<{layoutType}, {sourceType}> As{shortName}Span(");
            builder.AppendLine($"    this Span<{sourceType}> source)");
            builder.OpenBrace();
            builder.AppendLine($"return new ReadOnlyTraitSpan<{layoutType}, {sourceType}>(");
            builder.AppendLine($"    ref MemoryMarshal.GetReference(source),");
            builder.AppendLine($"    source.Length);");
            ...
            // --- 1D Mutable: AsXxxTraitSpan from Span<TSource> ---
            builder.AppendLine("/// <summary>");
            builder.AppendLine($"/// Creates a mutable trait span viewing {trait.Name} fields (optimized for {impl.TypeName}).");
            builder.AppendLine("/// </summary>");
            builder.AppendLine("[MethodImpl(MethodImplOptions.AggressiveInlining)]");
            builder.AppendLine($"public static TraitSpan<{layoutType}, {sourceType}> As{shortName}TraitSpan(");
            builder.AppendLine($"    this Span<{sourceType}> source)");
            builder.OpenBrace();
            builder.AppendLine($"return new TraitSpan<{layoutType}, {sourceType}>(");
            builder.AppendLine($"    ref MemoryMarshal.GetReference(source),");
            builder.AppendLine($"    source.Length);");
            builder.CloseBrace();
            builder.AppendLine();

            // --- 2D ReadOnly: AsXxxSpan2D from ReadOnlySpan<TSource> ---
            builder.AppendLine("/// <summary>");
            builder.AppendLine($"/// Creates a 2D read-only trait span (optimized for {impl.TypeName}).");
            builder.AppendLine("/// source.Length must equal width * height.");
            builder.AppendLine("/// </summary>");
            builder.AppendLine("[MethodImpl(MethodImplOptions.AggressiveInlining)]");
            builder.AppendLine($"public static ReadOnlyTraitSpan2D<{layoutType}, {sourceType}> As{shortName}Span2D(");
            builder.AppendLine($"    this ReadOnlySpan<{sourceType}> source, int width, int height)");
            builder.OpenBrace();
            builder.AppendLine("if (source.Length != width * height)");
            builder.AppendLine("    ThrowHelper.ThrowArgumentException_InvalidDimensions();");
            builder.AppendLine($"return new ReadOnlyTraitSpan2D<{layoutType}, {sourceType}>(");
            builder.AppendLine($"    ref MemoryMarshal.GetReference(source),");
            builder.AppendLine($"    width,");
            builder.AppendLine($"    height);");
            builder.CloseBrace();
            ...
            // --- 2D ReadOnly: AsXxxSpan2D from Span<TSource> ---
            builder.AppendLine("/// <summary>");
            builder.AppendLine($"/// Creates a 2D read-only trait span (optimized for {impl.TypeName}).");
            builder.AppendLine("/// source.Length must equal width * height.");
            builder.AppendLine("/// </summary>");
            builder.AppendLine("[MethodImpl(MethodImplOptions.AggressiveInlining)]");
            builder.AppendLine($"public static ReadOnlyTraitSpan2D<{layoutType}, {sourceType}> As{shortName}Span2D(");
            builder.AppendLine($"    this Span<{sourceType}> source, int width, int height)");
            builder.OpenBrace();
            builder.AppendLine("if (source.Length != width * height)");
            builder.AppendLine("    ThrowHelper.ThrowArgumentException_InvalidDimensions();");
            builder.AppendLine($"return new ReadOnlyTraitSpan2D<{layoutType}, {sourceType}>(");
            ...
            // --- 2D Mutable: AsXxxTraitSpan2D from Span<TSource> ---
            builder.AppendLine("/// <summary>");
            builder.AppendLine($"/// Creates a 2D mutable trait span (optimized for {impl.TypeName}).");
            builder.AppendLine("/// source.Length must equal width * height.");
            builder.AppendLine("/// </summary>");
            builder.AppendLine("[MethodImpl(MethodImplOptions.AggressiveInlining)]");
            builder.AppendLine($"public static TraitSpan2D<{layoutType}, {sourceType}> As{shortName}TraitSpan2D(");
            builder.AppendLine($"    this Span<{sourceType}> source, int width, int height)");
            builder.OpenBrace();
            builder.AppendLine("if (source.Length != width * height)");
            builder.AppendLine("    ThrowHelper.ThrowArgumentException_InvalidDimensions();");
            builder.AppendLine($"return new TraitSpan2D<{layoutType}, {sourceType}>(");
            builder.AppendLine($"    ref MemoryMarshal.GetReference(source),");
            builder.AppendLine($"    width,");
            builder.AppendLine($"    height);");
            builder.CloseBrace();
        ...
        /// <summary>
        /// Generates concrete span factory overloads for an external implementation with BaseOffset == 0.
        /// </summary>
        public static string? GeneratePerExternalImplementation(ExternalImplModel impl)
        {
            var trait = impl.Trait!;
            if (impl.BaseOffset != 0) return null;
            ...
            var shortName = trait.ShortName;
            var layoutType = $"{trait.EffectiveNamespace}.{trait.LayoutStructName}";
            var sourceType = impl.TargetFullTypeName;
